...
export function CartItems() {
  const { cart, changeQuantityTo, changeProgressTo } = useCartStore();
  ...
  return (
    <motion.div className="flex flex-col items-center">
      ...
      {cart.length > 0 && (
        <div className="max-h-80 w-full  overflow-y-auto">
          <Table className="max-w-2xl mx-auto">
            ...
            <TableBody>
              {cart.map((item) => (
                <TableRow key={(item.id + item.variant.id).toString()}>
                  ...
                  <TableCell>
                    <div className="flex items-center justify-between ">
                      <MinusCircle
                        onClick={() => {
                          changeQuantityTo(item.variant.id, (q) => q - 1);
                        }}
      ...
      <Button
        onClick={() => {
          changeProgressTo(CART_PROGRESS_STATE.PAYMENT_PAGE);
        }}
