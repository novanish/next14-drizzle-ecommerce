...
export const InputTags = forwardRef<HTMLInputElement, Props>(
  ({ onChange, value, ...props }, ref) => {
    ...
    const { setFocus } = useFormContext<VariantSchema>();

    function addPendingDataPoint(newValue: string) {
      const newDataPoints = new Set([...value, newValue]);
          ...
          <div className="flex">
            <Input
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === "Enter") {
                  e.preventDefault();
                  addPendingDataPoint(inputValue);
                  input.value = "";
                }
